...
#include "storage/rowset/column_iterator_wrapper.h"
...
class DictCodeColumnIterator final : public ColumnIteratorWrapper {
public:
    ...
    explicit DictCodeColumnIterator(ColumnId cid, ColumnIterator* iter)
            : ColumnIteratorWrapper(iter, kDontTakeOwnership), _cid(cid) {}

    ~DictCodeColumnIterator() override = default;

    ColumnId column_id() const { return _cid; }

    ColumnIterator* column_iterator() const { return _iter; }

    Status next_batch(size_t* n, Column* dst) override { return _iter->next_dict_codes(n, dst); }

    Status next_batch(const SparseRange<>& range, Column* dst) override { return _iter->next_dict_codes(range, dst); }
...
class GlobalDictCodeColumnIterator final : public ColumnIteratorWrapper {
public:
    using Column = starrocks::Column;
    using ColumnPredicate = starrocks::ColumnPredicate;
    ...
    explicit GlobalDictCodeColumnIterator(ColumnId cid, ColumnIterator* iter, int16_t* code_convert_data)
            : ColumnIteratorWrapper(iter, kDontTakeOwnership), _cid(cid), _local_to_global(code_convert_data) {}
    ...
    Status fetch_values_by_rowid(const rowid_t* rowids, size_t size, Column* values) override {
        ...
        RETURN_IF_ERROR(_iter->fetch_dict_codes_by_rowid(rowids, size, _local_dict_code_col.get()));
