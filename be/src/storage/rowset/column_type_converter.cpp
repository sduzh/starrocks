...
ColumnTypeConverter::ColumnTypeConverter(ColumnIterator* source_iter, LogicalType source_type, LogicalType target_type,
                                         bool nullable)
        : ColumnIteratorWrapper(source_iter, kTakesOwnership),
...
ColumnTypeConverter::~ColumnTypeConverter() = default;

void ColumnTypeConverter::convert(Column* target) {
    ...
    if (!target->is_nullable() && cast_result->is_nullable()) {
...
Status ColumnTypeConverter::next_batch(size_t* n, Column* dst) {
    _source_chunk.reset();
    auto source_column = _source_chunk.get_column_by_index(0);
    RETURN_IF_ERROR(_iter->next_batch(n, source_column.get()));
    convert(dst);
    return Status::OK();
}

Status ColumnTypeConverter::next_batch(const SparseRange<>& range, Column* dst) {
    _source_chunk.reset();
    auto source_column = _source_chunk.get_column_by_index(0);
    RETURN_IF_ERROR(_iter->next_batch(range, source_column.get()));
    convert(dst);
    return Status::OK();
}

Status ColumnTypeConverter::fetch_values_by_rowid(const rowid_t* rowids, size_t size, Column* values) {
    _source_chunk.reset();
    auto source_column = _source_chunk.get_column_by_index(0);
    RETURN_IF_ERROR(_iter->fetch_values_by_rowid(rowids, size, source_column.get()));
    convert(values);
    return Status::OK();
}
