...
class ColumnTypeConverter : public ColumnIteratorWrapper {
public:
    ...
    explicit ColumnTypeConverter(ColumnIterator* source_iter, LogicalType source_type, LogicalType target_type,
                                 bool nullable);

    ~ColumnTypeConverter() override;

    DISALLOW_COPY_AND_MOVE(ColumnTypeConverter);
    ...
    // Disable bloom filter in ColumnTypeConverter
